...
public class EarthquakeCityMap extends PApplet {
	...
	private boolean isLand(PointFeature earthquake) {
		...
		for (Marker m : countryMarkers) {
			inCountry = isInCountry(earthquake, m);
		}
	...
	private void printQuakes() 
	{
		...
		for(Marker cm : countryMarkers) {
			int quakeCounter = 0;
			final String cnt = (String) cm.getProperty("name");
			for(Marker quake : quakeMarkers) {
				EarthquakeMarker em = (EarthquakeMarker)quake;
				if(em.isOnLand) {
					if(quake.getProperty("country").equals(cnt)) {
						quakeCounter++;
					}
				} else {
					ocean++;
			...
			if(quakeCounter > 0) {
				System.out.println("Country " + cnt + " had: " + quakeCounter + " earthquakes.");
			}
		}
		...
		System.out.println("Earthquakes occured in the ocean: " + ocean);
	}
